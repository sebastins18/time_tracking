...
class Template {
    ...
    private function parseSingle( $key, $value, $string, $index = null ) {
      ...
      return str_replace($this->l_delim . $key . $this->r_delim, htmlspecialchars($value), $string);
  }
    ...
    private function parsePair( $variable, $data, $string ) {
      $string = $this->handleSection($variable, $data, $string);
      $string = $this->handleInverseSection($variable, $data, $string);
      return $string;
    }
  
  private function handleSection($variable, $data, $string) {
      ...
      if (!empty($data)) {
          foreach ($data as $row) {
              $temp = $match['1'];
              if (is_array($row)) {
                  // Manejar array de objetos/arrays asociativos
                  ...
                  // Manejar array de valores simples
                  $temp = $this->parseSingle('.', $row, $temp);
              }
              $str .= $temp;
          }
      } else {
          return str_replace($match[0], '', $string);  // Si no hay datos, la sección se elimina
      }
      return str_replace( $match['0'], $str, $string );
  }

  private function handleInverseSection($variable, $data, $string) {
      $pattern = "/" . preg_quote($this->l_delim . '^' . $variable . $this->r_delim, '/') . "(.*?)" . preg_quote($this->l_delim . '/' . $variable . $this->r_delim, '/') . "/s";
      return empty($data) ? preg_replace($pattern, '$1', $string) : preg_replace($pattern, '', $string);
  }
    ...
    private function matchPair( $string, $variable ) {
      $pattern = "|" . preg_quote($this->l_delim) . '#' . $variable . preg_quote($this->r_delim) . "(.+?)" . preg_quote($this->l_delim) . '/' . $variable . preg_quote($this->r_delim) . "|s";
...
class DB {
  ...
  private static function init() {
    ...
    if (empty($config['database']['db_driver']) || empty($config['database']['db_name'])) {
        die('Database configuration error: missing parameters.');
    }

    // Crear DSN para conexión PDO
    $dsn = "{$config['database']['db_driver']}:{$config['database']['db_name']}";

    try {
        self::$dbh = new PDO($dsn);  // Usa la propiedad estática para guardar la instancia PDO
  ...
  private static function conditions($conditions) {
    ...
    foreach ($conditions as $condition) {
        if (count($condition) !== 3) {
            throw new Exception("Each condition must have exactly three elements.");
        }
  ...
  public static function _update($params, $item) {
    if (!isset($params['table'], $params['where'], $params['values']) || empty($params['where'])) {
        throw new Exception("Missing parameters for the update operation.");
    }
    ...
    $whereOutput = self::conditions($params['where'], ' AND ');
    $query .= " WHERE " . $whereOutput['query'];

    $values = array_values($params['values']);
    $values = array_merge($values, $whereOutput['values']);
  ...
  public static function _delete($params) {
    if (!isset($params['where']) || empty($params['where'])) {
        throw new Exception("No WHERE condition provided for deletion.");
    }
    ...
    foreach ($params['where'] as $condition) {
        if (count($condition) !== 3) {
            throw new Exception("Each WHERE condition must consist of exactly three elements.");
        }
